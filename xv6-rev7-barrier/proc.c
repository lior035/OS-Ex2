...
mutex_acquire(int id){
	...
	loop:
		...
		proc->mutex_id[proc->mutex_acquired] = id;
		proc->mutex_acquired++;
		release(&ptable.lock);
...
barrier_join(int id){
	...
	cprintf("process id %d has joined barrier \n", proc->pid, proc->barrier_id);
	return (0);
}
...
int 
barrier_wait(void){
	if(proc->barrier_acquire == 0)	//cannot wait if not associated with any barrier
		return (-1);
	acquire(&ptable.lock);	//lock the process table
	struct proc *p;			// generic pointer to ptable processes
	
	// Proc is checking if there is any other process in the same barrier with him that isn't on wait, if he found's one, he goes to sleep.
	// once awaken, there's nothing left for the proc to do...
	for( p = ptable.proc; p < &ptable.proc[NPROC]; p++){
			if(p != proc){
				if(p->barrier_acquire == 1){
					if(p->barrier_id == proc->barrier_id){
						if(p->barrier_wait == 0){
							if( proc->killed){
								release (&ptable.lock);
								return(-1);
							}
							cprintf("process id %d found a process not waiting with same barrier ID like him %d,sleep\n", proc->pid, p->barrier_id);
							proc->barrier_wait = 1;
							sleep((void *)proc->barrier_id, &ptable.lock);
							release(&ptable.lock);
							cprintf("process id %d was awaken from sleep\n", proc->pid);
							return(0);
						}
					}
				}
	...
	// this is relevant only if the process see's that all other processes in the same barrier are on wait, then he awakens everybody else.
	for( p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p != proc){
				if(p->barrier_acquire == 1){
					if(p->barrier_id == proc->barrier_id){
						if(p->barrier_wait == 1){
							cprintf("process id %d is the last one to Wait on barrier %d waking process %d \n", proc->pid, proc->barrier_id ,p->pid);
							p->barrier_wait = 0;
						}
					}
				}
	...
	release(&ptable.lock);
	wakeup((void*)proc->barrier_id); // all proccess in this barrier are sleeping at this point and now we will wake them all up (they all sleep on barrier_id channel)
	cprintf("process id %d waked everybody in barrier %d - finished\n", proc->pid,proc->barrier_id);
	return (0);
}
